...
trait TypeOfMixin extends HasReflection {
  ...
  class TypeOf[T](
    ...
    lazy val showHierarchy: format.WithRefFormat.Group#Viz[VNode] = selfGroup
      .GraphRepr(Seq(nodes.SuperTypeNode))
      .showHierarchyWithRef(selfGroup.delegateGroup)
  ...
  case class VisualisationGroup() {
    ...
    lazy val delegateGroup: format.WithRefFormat.Group = format.WithRefFormat.Group()
    ...
    case class GraphRepr(override val roots: Seq[VNode]) extends Graph.Outbound[VNode] {
      ...
      case class Ops(node: VNode) extends OutboundNOps {
