...
object MultiMaps {
  ...
  trait Impl[K, +V] {
    ...
    protected def mergeProto[V2 >: V](other: Impl[K, V2]): MultiMaps.Mutable[K, V2] = {

      val buffer: MultiMaps.Mutable[K, V2] = mutable.HashMap.empty[K, Seq[V2]]
    ...
    def merge[V2 >: V](other: Impl[K, V2]): MultiMaps.Mutable[K, V2] = {

      val buffer = mergeProto(other)
      buffer
    }

    def +:+[V2 >: V](other: Impl[K, V2]): MultiMaps.Mutable[K, V2] = merge(other)
  }
  ...
  implicit def toSelf[K, V](v: Impl[K, V]): Self[K, V] = v.self
  ...
  class Immutable[K, V](override val self: Self[K, V]) extends Impl[K, V] {}

  class Mutable[K, V](override val self: MSelf[K, V]) extends Immutable[K, V](self) with Impl[K, V] {
