...
trait NodeK[+L <: Conj] extends Lawful.Struct[L] {
...
object NodeK {
  ...
  type Aux[+L <: Conj, V] = NodeK[L] { type Value = V }
  trait Impl[+L <: Conj, V] extends NodeK[L] { type Value = V }

  type Compat[+L <: Conj, +V] = Aux[L, _ <: V]

  trait Untyped[+L <: Conj] extends NodeK[L] {
