...
/**
  ...
  */
case class RefGroup[V]() {

  import RefGroup.*

  trait RefNode {

    def original: Diverging.Graph.Node[V]
  }
  ...
  def convertNode(source: Diverging.Graph.Node[V]): Node = RefDomain.node(source)

  def convertGraph(source: Local.Diverging.Graph[V]): Local.Diverging.Poset.Graph[RefDomain.node] = {
    // TODO: more engine
  ...
  object RefDomain extends Diverging.Poset.Codomain {
    ...
    case class node(
        override val original: Diverging.Graph.Node[V],
      ...
      override lazy val nodeText: String = {
        ...
        lazy val originalText = original.nodeText
