...
object __CapAndPendingDesign {
  ...
  /**
    ...
    *
    *   - Tag ([[ai.acyclic.prover.commons.tag.Tagged]]) and Pending Effect (Pending,
    *     [[ai.acyclic.prover.commons.tag.Pending]]) are compile-time only mixin that are erased at runtime
