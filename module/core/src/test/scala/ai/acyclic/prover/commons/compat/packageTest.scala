package ai.acyclic.prover.commons.compat

import ai.acyclic.prover.commons.compat.NamedTupleX.:=
import ai.acyclic.prover.commons.compat.TupleX._1
import ai.acyclic.prover.commons.testlib.BaseSpec

object packageTest {

  val tx = TupleX.of(1, "a")
  val ntx = (Key["a"] := 1) *: (Key["b"] := "x") *: _1
}

class packageTest extends BaseSpec {

  describe("define") {

    it("Tuple") {

      val t0 = 1 *: "a" *: _1
      val _ = t0: Int *: String *: _1

      val t1 = TupleX.of(1, "a")
      val _ = t1: Int *: String *: _1
      val _ = t1: 1 *: "a" *: _1 // TODO: how does this work?

      val t2 = TupleX.ofNarrow(1, "a")
      val _ = t2: Int *: String *: _1
      val _ = t2: 1 *: "a" *: _1
    }

    it("NamedTuple") {

      val t0 = (Key["a"] := 1) *: (Key["b"] := "x") *: _1
      val _ = t0: ("a" := Int) *: ("b" := String) *: _1

      val t1 = NamedTupleX.of(a = 1, b = "x")
      val _ = t1.asInstanceOf[("a" := Int) *: ("b" := String) *: _1]

      import shapeless.record.recordOps

      val aa = t1.record.a
      val _ = aa: Int

      assert(aa == 1)
    }
  }
}
